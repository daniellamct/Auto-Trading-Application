...
def create_model(un, bs, epo, do):
    ...
    #Adding LSTM layers
    regressorLSTM.add(layers.LSTM(un, return_sequences = True, input_shape = (X_train.shape[1], 7)))
    regressorLSTM.add(layers.Dropout(do))
    regressorLSTM.add(layers.LSTM(un, return_sequences = True))
    regressorLSTM.add(layers.Dropout(do))
    regressorLSTM.add(layers.LSTM(un, return_sequences = False))
    ...
    #Fitting the model
    regressorLSTM.fit(X_train, y_train, batch_size = bs, epochs = epo)
        ...
        for e in epochs:
            for d in dropout_rate:
                try:
                    ...
                    regressorLSTM = create_model(u, b, e, d)
