...
def fun_PP(stock):
    ...
    try:
        ...
        data = yf.download(stock, new_start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d'),progress=False)
        vreturn=[]
        ...
        # ANN Prediction
        ANN_model = keras.models.load_model('Trained_Models/ANN_model.h5')
        ...
        vreturn.append([#"ANN: ",
                        f'Latest Closing Price: {future_predict_price_ann[0]:.2f}',
                        f'Future Closing Price (Next Day): {future_predict_price_ann[1]:.2f}',
                        f'Future Closing Price (Two Days Ahead): {future_predict_price_ann[2]:.2f}',
                        f'Future Closing Price (Three Days Ahead): {future_predict_price_ann[3]:.2f}',])######################
        ...
        plt.savefig("temp/PP0.png")

        # Backtesting for ANN prediction
        ...
        vreturn.append([
                        ...
                        f'{false_P_ann}',
                        f'{false_N_ann}',
                        f'{((true_P_ann+true_N_ann)/(true_P_ann+true_N_ann+false_P_ann+false_N_ann)):.2f}'])
        ...
        vreturn.append([f'Total trades: {trade_ann} (In {real_change.shape[0]} days)',
                        f'Total Earned: {earn_ann:.2f}'])

        # LSTM Prediction
        LSTM_model = keras.models.load_model('Trained_Models/LSTM_model.h5')
        ...
        vreturn.append([#"LSTM: ",
                        f'Latest Closing Price: {future_predict_price_lstm[0]:.2f}',
                        f'Future Closing Price (Next Day): {future_predict_price_lstm[1]:.2f}',
                        f'Future Closing Price (Two Days Ahead): {future_predict_price_lstm[2]:.2f}',
                        f'Future Closing Price (Three Days Ahead): {future_predict_price_lstm[3]:.2f}',
                        ])

        # Plot the Graph for LSTM
        plt.figure(figsize=(16, 6))
        ...
        plt.savefig("temp/PP1.png")

        # Backtesting for LSTM prediction
        ...
        vreturn.append([
                    f'{true_P_lstm}',f'{true_N_lstm}',
                    f'{false_P_lstm}',f'{false_N_lstm}',
                    f'{((true_P_lstm+true_N_lstm)/(true_P_lstm+true_N_lstm+false_P_lstm+false_N_lstm)):.2f}'])
        ...
        vreturn.append([f'Total trades: {trade_lstm} (In {real_change.shape[0]} days)', f'Total Earned: {earn_lstm:.2f}'])

        return vreturn
        
    except:
        print("except within script -- Price_Prediction.py")
        return []
